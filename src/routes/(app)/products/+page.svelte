  ...
  <div class="max-w-7xl mx-auto">
    ...
    <div class="flex flex-col lg:flex-row gap-6">
      ...
      <aside class="lg:w-64 flex-shrink-0">
        <div
          class="bg-[#0c0c0c] border border-white/5 rounded-2xl p-5 space-y-5 sticky top-6"
        >
      ...
      <main class="flex-1">
        ...
        {#if loading && products.length === 0}
          <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4">
            {#each { length: 8 } as _}
              <div
                class="aspect-square rounded-2xl bg-[#0c0c0c] border border-white/5 animate-pulse"
              ></div>
            {/each}
          </div>
        ...
        {:else}
          <!-- Product Grid -->
          <div
            class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-3 xl:grid-cols-4 gap-4 md:gap-6"
          >
            {#each products as product}
              <TiltCard maxTilt={25}>
                ...
                >
                  <!-- Thumbnail -->
                  <div
                    class="relative aspect-square overflow-hidden bg-[#0a0a0a]"
                  >
                    ...
                    {:else}
                      <div
                        class="w-full h-full flex items-center justify-center bg-[#1a1a1a]"
                      >
                    ...
                    {#if product.status === "SOLD"}
                      <div
                        class="absolute top-2 left-2 px-2 py-0.5 rounded-md text-[9px] font-black leading-none tracking-wide shadow-lg bg-red-500 text-white"
                      >
                        HABIS
                      </div>
                    {:else if product.status === "AVAILABLE"}
                      <div
                        class="absolute top-2 left-2 px-2 py-0.5 rounded-md text-[9px] font-black leading-none tracking-wide shadow-lg bg-green-500 text-white"
                      >
                        READY
                      </div>
                    {/if}
                  ...
                  <div class="p-3">
                    <h3
                      class="text-sm font-bold text-white leading-tight line-clamp-1 mb-1 group-hover:text-[var(--color-primary)]"
                    >
                      {product.title}
                    </h3>
                    <p
                      class="text-xs text-white/50 line-clamp-2 mb-2 min-h-[2rem]"
                    >
                    ...
                    <div class="flex items-center justify-between">
                      <div>
                        ...
                        <p
                          class="text-[14px] font-black text-[var(--color-primary)]"
                        >
                  ...
                  <div
                    class="absolute bottom-0 left-0 right-0 h-[2px] bg-[var(--color-primary)] opacity-0 group-hover:opacity-100 transition-opacity duration-300"
                  ></div>
                </a>
              </TiltCard>
            {/each}
          ...
          {#if totalPages > 1}
            <div
              class="mt-10 flex flex-col md:flex-row gap-3 md:gap-0 md:items-center md:justify-between border-t border-white/5 pt-6 text-sm"
            >
