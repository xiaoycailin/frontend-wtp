<script lang="ts">
  ...
  async function fetchCategories() {
    loading = true;
    try {
      const res = await fetch("http://192.168.18.217:3000/article-categories");
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const url = form.id
        ? `http://192.168.18.217:3000/article-categories/${form.id}`
        : "http://192.168.18.217:3000/article-categories";
...
<div class="space-y-6">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
    <div class="lg:col-span-1 bg-[#0b0b0b] border border-white/5 rounded-xl p-5 space-y-4">
      <h2 class="text-lg font-semibold text-white">{form.id ? 'Edit Kategori' : 'Tambah Kategori'}</h2>
      <div>
      ...
      <div>
        ...
        <input bind:value={form.thumbnail} class="w-full px-4 py-2.5 bg-white/5 border border-white/10 rounded-lg text-white" />
      </div>
      ...
      <table class="w-full">
        <thead class="bg-white/5">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs text-white/60">Description</th>
            <th class="px-4 py-3 text-right text-xs text-white/60">Aksi</th>
          </tr>
        </thead>
        <tbody class="divide-y divide-white/5">
          {#if loading}
            <tr><td colspan="4" class="px-4 py-8 text-center text-white/40">Loading...</td></tr>
          {:else if categories.length === 0}
            <tr><td colspan="4" class="px-4 py-8 text-center text-white/40">Belum ada kategori</td></tr>
          {:else}
            {#each categories as category}
              <tr class="hover:bg-white/5">
                <td class="px-4 py-4 text-white">{category.name}</td>
                <td class="px-4 py-4 text-white/70">{category.slug}</td>
                <td class="px-4 py-4 text-white/60">{category.description || '-'}</td>
                <td class="px-4 py-4 text-right">
                  <button onclick={() => editCategory(category)} class="text-xs text-[#f5c518]">Edit</button>
                </td>
              </tr>
            {/each}
          {/if}
