<script lang="ts">
  ...
  async function handleSubmit() {
    ...
    try {
      ...
      const json = await res.json();
      if (!res.ok) {
        alert(json.data?.message || "Gagal simpan tag");
        return;
      }
...
<div class="space-y-6">
  <div>
    <h1 class="text-2xl font-bold text-white">Tag Artikel</h1>
    <p class="text-sm text-white/50 mt-1">Kelola tag dan label artikel</p>
  </div>
      ...
      <table class="w-full">
        <thead class="bg-white/5">
          <tr>
            ...
            <th class="px-4 py-3 text-left text-xs text-white/60">Color</th>
            <th class="px-4 py-3 text-left text-xs text-white/60">Featured</th>
